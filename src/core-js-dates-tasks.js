...
function getNextFriday(date) {
  ...
  let toFriday = (5 - getDate.getDay() + 7) % 7;
  if (toFriday === 0) {
    toFriday = 7;
  }
  getDate.setDate(getDate.getDate() + toFriday);
  return getDate;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.abs(Math.floor(dif / (1000 * 60 * 60 * 24))) + 1;
}
...
function getWeekNumberByDate(date) {
  ...
  const toMonday = (firstDayOfYear.getDay() + 6) % 7;
  const totalDays = Math.floor((date - firstDayOfYear) / (24 * 60 * 60 * 1000));
  return Math.floor((totalDays + toMonday) / 7) + 1;
}
...
function getNextFridayThe13th(date) {
  for (let i = 0; i < 12 - date.getMonth(); i += 1) {
    const newDate = new Date(date.getFullYear(), date.getMonth() + i, 13);
    if (newDate.getDay() === 5) {
      return newDate;
    }
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const work = countWorkDays;
  const off = countOffDays;
  const res = [];
  const currentDate = startDate;

  while (currentDate <= endDate) {
    for (let i = 0; i < work && currentDate <= endDate; i += 1) {
    ...
    for (let i = 0; i < off && currentDate <= endDate; i += 1) {
      currentDate.setDate(currentDate.getDate() + 1);
    }
  }
